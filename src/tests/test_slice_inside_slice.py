...
def test_slice_inside_slice_similar_names():
    ...
    slice_outer_copy.from_json(slice_outer.to_dict())
    ...
    assert slice_mid.to_dict(False) == slice_mid.to_dict(True)
    slice_mid.save_to_file(mid_temp_file.name, overwrite=True)
    assert slice_mid.to_dict(False) != slice_mid.to_dict(True)
    interest = json.loads(slice_outer.to_dict())['bakery_items']['slice_mid']['bakery_item']
    ...
    slice_outer_file.from_json(slice_outer.to_dict())
